...
const updateContractAddressSecret = async (address) => {
  console.log("🛠️ Updating the Secret with the new contract address");

  // ** Clean the address: remove newlines, spaces, and invalid characters **
  address = address.trim().replace(/\s+/g, ''); // Remove leading/trailing whitespace and collapse spaces
  console.log(`🔍 Cleaned Contract Address: ${address}`);
